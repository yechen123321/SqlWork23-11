...
app.post('/comments', (req, res) => {
  conn.query(`select reader.email,comment.status,comment.commentId,
    ...
    left join reader on comment.readerId=reader.readerId left join book on comment.bookId=book.bookId where comment.status = 1`, (err, rs) => {
    let data = rs || []
    // console.log("评论区：",rs)
    if (data.length > 0) {
      res.json({
...
app.post('/addcomment', (req, res) => {
  ...
  conn.query(`insert into comment values('${commentId}','${data.readerId}','${data.bookId}',now(),'${data.content}',0,1)`)
  res.send({
    msg: '评论成功！',
    status: 200
  })
...
app.post('/addpraise', (req, res) => {
  ...
  conn.query(`update comment set praise = praise+1 where readerId='${data.readerId}' and bookId = '${data.bookId}' and date = '${formatDate(date)}'`)
  res.send({
    msg: '点赞成功！',
    status: 200
  })
