...
router.post('/deleteborrow', (req, res) => {
	...
	conn.query(`delete from borrow where readerId='${data.readerId}' 
    	and bookId='${data.bookId}' and borrowDate='${formatDate(borrowDate)}'`)
	res.json({
		msg: '管理员删除借阅记录成功',
		status: 200,
	})
...
router.post('/auditcomment', (req, res) => {
	...
	//   管理员自行删除评论
	if (data.status == 3) {
		conn.query(`update comment set status=0 where readerId='${data.readerId}' and bookId='${data.bookId}' and date='${formatDate(date)}'`)
		res.send({
			msg: '删评成功！',
			status: 200
		})
	...
	//  删评
		...
		conn.query(`update comment set status=0 where readerId='${data.readerId}' and bookId='${data.bookId}' and date='${formatDate(date)}'`)
		conn.query(`update report set status='已通过' where commentId='${data.commentId}' and reporterId='${data.reporterId}' and reportdate='${formatDate(reportDate)}'`)
		// 邮件信息
		let mailobj = {
			from: userEmail, // 发送者QQ邮箱
		...
		conn.query(`update report set status='已驳回' where commentId='${data.commentId}' and reporterId='${data.reporterId}' and reportdate='${formatDate(reportDate)}'`)
		// 邮件信息
		let mailobj = {
